...
class Tile {
  ...
  fillEdges(filter) {
    this.edges.push(new Edge(this.img, 0, filter).id)
    this.edges.push(new Edge(this.img, 1, filter).id)
    this.edges.push(new Edge(this.img, 2, filter).id)
    this.edges.push(new Edge(this.img, 3, filter).id)
  }
  ...
  rotate(num) {
    ...
    return new Tile(newImg, this.notSelf, this.index);
  }
}
